...
fn filter_frames(buffer: &mut [u8], target_ip: &[u8]) -> bool {
...
fn get_target_mac(buffer: &mut [u8], target_ip: &[u8]) -> Result<[u8; 6], smoltcp::Error> {
	if filter_frames(buffer, target_ip) {
...
pub fn capture(socket: &mut RawSocket, target_ip: &[u8]) -> [u8; 6] {
	loop {
		wait(socket.as_raw_fd(), None).unwrap();
		let (rx, _) = socket.receive().unwrap();
		match rx.consume(Instant::now(), |buffer| get_target_mac(buffer, target_ip)) {
			Ok(mac) => return mac,
